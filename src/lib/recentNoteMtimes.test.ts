...
describe('collectGitNoteMtimes', () => {
  ...
  it('does not stamp notes with the oldest shallow commit when its parent is missing', async () => {
    const fs = createFsFromVolume(new Volume())
    const dir = '/repo'
    const author = { name: 'Notes', email: 'notes@test' }
    await fs.promises.mkdir(dir)
    ...
    await fs.promises.writeFile(`${dir}/recent.md`, 'c')
    await git.add({ fs, dir, filepath: 'recent.md' })
    await git.commit({
      fs,
      dir,
      message: 'update recent again',
      author: { ...author, timestamp: 3_000 },
      committer: { ...author, timestamp: 3_000 },
    })

    await fs.promises.writeFile(`${dir}/.git/shallow`, `${second}\n`)
    await fs.promises.unlink(`${dir}/.git/objects/${first.slice(0, 2)}/${first.slice(2)}`)

    const times = await collectGitNoteMtimes({ fs, dir, depth: 10 })
    expect(times['recent.md']).toBe(3_000_000)
    expect(times['old.md']).toBeUndefined()
  })
})
